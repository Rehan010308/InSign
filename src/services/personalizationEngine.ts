/**
 * Personalization engine — deterministic, local, and honest.
 *
 * It looks only at sessions the user actually saved, and it refuses to claim a
 * pattern before there is enough evidence for one. No model, no randomness, no
 * network: given the same sessions it always returns the same result, which is
 * what makes it unit-testable and what makes the recommendation defensible.
 */
...
export const INSUFFICIENT_HISTORY: PersonalizationResult = {
  pattern: null,
  scenario: null,
  confidence: 0,
...
export function estimateWords(s: SpeechSession): number {
  return Math.max(1, Math.round((s.words_per_minute * s.duration_ms) / 60000));
}

function withinWindow(s: SpeechSession, now: number): boolean {
 ...
 */
export function evidenceConfidence(sampleSize: number, effectRatio: number): number {
...
export function detectPattern(
  ...
  if (recent.length < MIN_SESSIONS_PER_SCENARIO) return INSUFFICIENT_HISTORY;

  const byScenario = new Map<Scenario, SpeechSession[]>();
  ...
  if (!eligible.length) return INSUFFICIENT_HISTORY;

  const candidates: PersonalizationResult[] = [];
...
function rate(sessions: SpeechSession[], pick: (s: SpeechSession) => number): number {
...
export function weeklyActivity(sessions: SpeechSession[], weeks = 6, now = Date.now()): number[] {

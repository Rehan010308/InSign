...
export function initLandingInteractions(): () => void {
  const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;
  const isTouch = matchMedia('(hover: none)').matches;
  const $ = <T extends Element>(s: string, c: ParentNode = document) => c.querySelector<T>(s);
  const $$ = <T extends Element>(s: string, c: ParentNode = document) => [...c.querySelectorAll<T>(s)];
  ...
  const pumpSet = new Map<Element, { cb: () => void; threshold: number; io: IntersectionObserver | null }>();

  function inView(el: Element, threshold: number) {
  ...
  function whenVisible(el: Element | null, cb: () => void, threshold = 0.3) {
    if (!el) return;
    let done = false;
    const fire = () => { if (done) return; done = true; pumpSet.delete(el); cb(); };
    let io: IntersectionObserver | null = null;
    if (typeof IntersectionObserver === 'function') {
      io = new IntersectionObserver(es => es.forEach(e => { if (e.isIntersecting) fire(); }), { threshold });
      io.observe(el);
      disposers.push(() => io?.disconnect());
    }
    pumpSet.set(el, { cb: fire, threshold, io });
    nextFrame(() => { if (inView(el, threshold)) fire(); });
  }
  ...
  function pump() {
    ...
    for (const [el, rec] of [...pumpSet]) {
      if (inView(el, rec.threshold)) { rec.io?.disconnect(); rec.cb(); pumpSet.delete(el); }
    }
  }
  function queuePump() {
    if (!pumpQueued && pumpSet.size) { pumpQueued = true; nextFrame(pump); }
  }
  ...
  /* ---------------- nav ---------------- */
  const nav = $('#nav');
  if (nav) on(window, 'scroll', () => nav.classList.toggle('scrolled', scrollY > 80), { passive: true });
  ...
  const story = $<HTMLElement>('#speech-story');
  if (story) {
    const orb = $('#story-orb');
    const waveSvg = $<SVGSVGElement>('#story-wave');
    ...
    const spine = $<HTMLElement>('.sflow-spine', story);
    ...
    const update = () => {
      ...
      if (progress !== target) syncRaf = nextFrame(update);
    };
    const queueUpdate = () => { if (!syncRaf) syncRaf = nextFrame(update); };
    on(window, 'scroll', queueUpdate, { passive: true });
    on(window, 'resize', queueUpdate);

    const waveTick = (now: number) => {
      const t = now / 1000;
    ...
    const waveStart = () => { if (waveOn) return; waveOn = true; waveRaf = nextFrame(waveLoop); };
    disposers.push(() => { waveOn = false; cancelAnimationFrame(waveRaf); cancelAnimationFrame(syncRaf); });
    on(window, 'scroll', () => {
      const r = story.getBoundingClientRect();
      if (r.top < innerHeight + 60 && r.bottom > -60) waveStart();
    }, { passive: true });
  ...
  const stage = $<HTMLElement>('.mv-stage');
  const cv = $<HTMLCanvasElement>('#mv-canvas');
  if (stage && cv) {
    const ctx = cv.getContext('2d')!;
    const result = $('#mv-result');
    const confFill = $<HTMLElement>('#mv-conf-fill');
    const steps = $$('.mv-step');
    ...
    (() => {
      ...
      const ax = (t: number) => 0.06 + t * 0.88;
      const ay = (t: number) => 0.78 - 0.52 * t - 0.16 * Math.sin(t * Math.PI);
      for (let i = 0; i <= N; i++) {
    ...
    const draw = (p: number) => {
      ...
      const px = (pt: number[]) => [pt[0] * W, pt[1] * H];
      if (p > 0.001) {
        ctx.beginPath();
        raw.forEach((pt, i) => { const [x, y] = px(pt); if (i) ctx.lineTo(x, y); else ctx.moveTo(x, y); });
      ...
      if (p > 0.02) {
        ctx.beginPath();
        for (let i = 0; i < upTo; i++) { const [x, y] = px(smooth[i]); if (i) ctx.lineTo(x, y); else ctx.moveTo(x, y); }
    ...
    const loop = () => {
      ...
      draw(shown);
      if (shown !== target) raf = nextFrame(loop); else raf = 0;
    };
    ...
    if (replay) on(replay, 'click', () => { shown = 0; target = 1; go(); });
  ...
  if (contact) whenVisible(contact, () => contact.classList.add('played'), 0.45);

  /* ---------------- parallax ---------------- */
  if (!reduced && !isTouch) {
    const panels = $$<HTMLElement>('.story, .movement, .pipe');
    let ticking = false;
    const parallax = () => {
      ticking = false;
      const vh = innerHeight;
    ...
    on(window, 'scroll', () => {
      if (!ticking) { ticking = true; nextFrame(parallax); queuePump(); }
    }, { passive: true });
    parallax();
  }
